...
class DomainReadController extends AbstractBackendForwardingController
{
     ...
     */
    #[PublicRoute('Public Credential Hub bootstrap route that creates the domain-read process and returns the initial QR/session payload.')]
    #[Route('/qr-identity', name: ProcessKey::DOMAIN_READ_QR_IDENTITY, methods: "POST")]
    public function domainReadQrIdentity(
        Request $request,
        BackendForwardingService $backendForwardingService
    ): JsonResponse {
        return $this->forwardProcessRequest($request, $backendForwardingService, $this->logger, ProcessKey::DOMAIN_READ_QR_IDENTITY);
    }
     ...
     * Tries to decode backend response as JSON and returns it; if not valid JSON, result may be null or incomplete.
     */
    #[ExtensionAuthRequired('Domain-read credential bootstrap continuation must include the X-Extension-Auth header established by the bootstrap step.')]
    #[Route('/credential/decrypted', name: ProcessKey::DOMAIN_READ_CREDENTIAL_ENCRYPTED, methods: "POST")]
    public function domainReadCredentialEncrypted(
        Request $request,
        BackendForwardingService $backendForwardingService
    ): JsonResponse {
        return $this->forwardProcessRequest($request, $backendForwardingService, $this->logger, ProcessKey::DOMAIN_READ_CREDENTIAL_ENCRYPTED);
    }
     ...
     * Tries to decode backend response as JSON and returns it; if not valid JSON, result may be null or incomplete.
     */
    #[ExtensionAuthRequired('Domain-read credential submission must include the X-Extension-Auth header established by the bootstrap step.')]
    #[Route('/credential', name: ProcessKey::DOMAIN_READ_CREDENTIAL, methods: "POST")]
    public function domainReadCredential(
        Request $request,
        BackendForwardingService $backendForwardingService
    ): JsonResponse {
        return $this->forwardProcessRequest($request, $backendForwardingService, $this->logger, ProcessKey::DOMAIN_READ_CREDENTIAL);
    }
    ...
    * Tries to decode backend response as JSON and returns it; if not valid JSON, result may be null or incomplete.
    */
    #[ExtensionAuthRequired('Domain-read state polling must include the X-Extension-Auth header established by the bootstrap step.')]
    #[Route('/state', name: ProcessKey::DOMAIN_READ_STATE, methods: "POST")]
